...
# Run this to see the results of ResNet
...
train_transforms = v2.Compose([
    v2.Grayscale(num_output_channels=3), # trick: 1-channel grey to 3-channel grey as the model is for rgb
    v2.Resize((256, 256)), # original size (1024, 1024) is too big for the model, resize (by default bilinear interpolation)
    ...
    v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), # the standard normalization values for models pre-tarined on ImageNe
])
...
for fold, (train_idx, val_idx) in enumerate(skf.split(df_labels, df_labels['label'])):
    ...
    model, criteria, optimizer = init_model_config(model='resNet')
